...
- `GUIDE.md`：说明本次运行实际生成了哪些文件，以及应按什么顺序检查；
...
1. 运行命令，先观察输出，不看实现。
2. **先读实验目录中的 `GUIDE.md`**。它解释本次运行真实生成的文件、路径和检查顺序。
3. **再读对应的静态讲义**：[第 1 周：收益与风险](docs/lessons/week01.md)。讲义解释理论、字段语义、实验方法和常见误区，不代替对本次真实产物的检查。
4. 按 `GUIDE.md` 打开图表和 CSV，尝试解释每一列。
5. 自己重新实现累计净值与最大回撤，不直接复制 `metrics.py`。
6. 完成 `homework.md`。
7. 按 `acceptance.json` 逐项检查，把作业、代码和“不理解的问题”发给我审阅。
8. 审阅通过后执行 `python scripts/learn.py 1 --mark-reviewed`。
9. 确认状态为 `reviewed` 后，再开始 [第 2 周讲义](docs/lessons/week02.md)：
